...
export class ReturnService {
  ...
  async loadAllMyReturns(lastDoc?: ReturnObj, firstDoc?: ReturnObj, refreshFromFirstDoc?: ReturnObj) : Promise<ReturnObj[]> {

    // Get all supplier's returns, ordered from new to old
    let ref = this.returnsCollectionRef
      .where('sid', '==', this.businessService.myBid)
      .where('product.returnStatus', '>', ReturnStatus.TRASH)
      .orderBy('product.returnStatus')
      .orderBy('product.returnTime', 'desc')
      .orderBy('id');
    ...
    if(lastDoc)
      ref = ref.startAfter(lastDoc.product.returnStatus, lastDoc.product.returnTime, lastDoc.id);
    if(firstDoc)
      ref = ref.endBefore(firstDoc.product.returnStatus, firstDoc.product.returnTime, firstDoc.id).limitToLast(10);
    else
      ref = ref.limit(10);

    try {
      const res = await ref.get();
      return res.docs.map((d)=>d.data() as ReturnObj);
    }
    catch (e) {
      console.error(e);
      return [];
  ...
  async loadDraft(id: string) : Promise<ReturnObj> {
    try {
      return (await this.returnsDraftsCollectionRef.doc(id).get()).data() as ReturnObj;
    }
    catch (e) {
      console.error(e);
    }
  ...
  async queryDrafts(sid?: string, q?: string) : Promise<ReturnObj[]> {


    let ref = q ? this.returnsDraftsCollectionRef.orderBy('product.name') : this.returnsDraftsCollectionRef;
    ...
    if(q) {
      ...
      if(order)
        ...
        ref = ref
          .where('product.name', '>=', q).where('product.name', '<', Dictionary.NextLastLetter(q))
          .orderBy('orderId');
  ...
  addDoc(doc: ReturnObj, noSave?: boolean) {
  ...
  async sendListToSupplier() : Promise<boolean> {
    ...
    this.returnsDocsList.forEach((draft)=>{
      ...
      draft.product.returnTime = Date.now();

      // Save in the draft in the returns collection (stamp with customer ID and time)
      batch.set(this.returnsCollectionRef.doc(draft.id), draft);
  ...
  static ReturnID(returnDoc: ReturnObj) : string {
